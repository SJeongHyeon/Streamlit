...
# [왜] 팔레트 상수 — 이 교안 공통 배색(TEAL=긍정 #0E6B56, CORAL=부정 #993C1D)의 RGB값.
#      표 배경색·단어 하이라이트에서 rgba(r,g,b,투명도) 형태로 재사용하려고 튜플로 둔다.
TEAL_RGB = (14, 107, 86)
CORAL_RGB = (153, 60, 29)

# [실측] 내장 라벨 20문장 벤치마크 — "정답을 미리 아는 시험"으로 모델의 진짜 정답률을 잰다.
#        영화 리뷰 14개(모델의 학습 도메인, NSMC) + 쇼핑몰 리뷰 4개(도메인 밖) + 애매한 문장 2개.
...
BENCH_SENTENCES = [
    ...
    # [왜] 도메인이탈 4문장 — 모델은 "영화 리뷰"(NSMC)로만 학습됐다. 쇼핑몰 리뷰는 한 번도
    #      본 적 없는 낯선 도메인이다 — 여기서 무슨 일이 벌어지는지가 오늘의 핵심 질문이다.
...
def main():
    ...
    with tab_bench:
        ...
        st.markdown(
            ...
            "애매한 문장 2)을 일괄 채점합니다. 영화 리뷰는 모델의 학습 도메인(NSMC)과 같고, "
            "쇼핑몰 리뷰는 **한 번도 본 적 없는 도메인**입니다."
        )
