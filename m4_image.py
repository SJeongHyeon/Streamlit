...
def main():
    ...
    if source is not None:
        ...
        if st.button("🧩 가려보기 히트맵 계산 (5~6초 소요)"):  # 버튼을 눌러야만 아래 블록이 실행된다
            ...
            # [왜] 랜덤 베이스라인 — "확률 9.7%가 낮아 보이지만 실은 압도적"이라는 감각을
            #      숫자로 확인시키기 위한 비교 기준(1000종 중 무작위로 하나를 찍었을 때의 확률).
            random_baseline = 1 / len(categories)
            st.caption(
                f"정답 {baseline_prob * 100:.1f}%도 랜덤({random_baseline * 100:.1f}%)의 "
                f"{baseline_prob / random_baseline:.0f}배 — 확률 자체는 낮아 보여도 무작위보다 압도적으로 높습니다."
            )
            # [왜] 정직 각주 — 이 기법의 한계를 먼저 말해두지 않으면 학생들이 "이게 정답"이라고
            #      과신하게 된다. expander로 접어둬 필요한 사람만 펼쳐 보게 한다.
            with st.expander("⚠️ 정직한 한계 — occlusion은 Grad-CAM의 근사일 뿐"):
                st.markdown(
                    "이 히트맵은 **occlusion(가려보기)** 기법입니다 — 실제로 칸을 가려보고 확률이 "
                    "얼마나 떨어지는지 재는, 가장 단순한 설명가능AI(XAI) 방법입니다. 반면 **Grad-CAM**은 "
                    "gradient(역전파)를 한 번만 계산해 더 정밀하고 빠른 히트맵을 만듭니다. occlusion은 "
                    f"{GRID}x{GRID} 격자 해상도만큼만 정보가 있어 경계가 뭉툭하고, 정사각형 격자가 실제 "
                    "물체 모양과 안 맞을 수 있습니다 — 더 정밀한 설명이 필요하면 심화로 Grad-CAM(예: "
